...
async fn main() -> Result<()> {
    ...
    // For examples that fetch data from third-party APIs we require API_KEY to avoid leaking credentials.
    #[cfg(any(feature = "weather-example", feature = "twitter-example"))]
    let api_key = std::env::var("API_KEY").expect("API_KEY must be set");

    #[cfg(all(not(feature = "weather-example"), not(feature = "twitter-example")))]
